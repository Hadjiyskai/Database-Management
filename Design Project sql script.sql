...
CREATE OR REPLACE FUNCTION addOwnerAnimal() 
RETURNS trigger AS $BODY$
BEGIN
	IF NEW.ownid <> OLD.ownid THEN
	INSERT INTO owners(ownid)
		VALUES(NEW.ownid);
	END IF;
	IF NEW.aid <> OLD.aid THEN
	INSERT INTO animals(aid)
		VALUES(NEW.aid);
	END IF;
	RETURN NEW;
END;
...
DROP TRIGGER IF EXISTS addOwnerAnimal on animalOwn;
--triggers
CREATE TRIGGER addOwnerAnimal
BEFORE INSERT ON animalOwn
FOR EACH ROW
EXECUTE PROCEDURE addOwnerAnimal();
